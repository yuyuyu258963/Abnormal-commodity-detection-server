...
def run_priceModels():
  ...
  deriveData = [filteredJiao[deriverName].tolist() for deriverName in cols]
  resData = {
    "data":                     data,
    "isf_name":                 "ISOF",
    ...
    "cblof_percent":            str(round(len(100 * cb_lof_result) * 100 / perDataLen,3))[:6] + "%",
...
def run_saleModels():
  ...
  deriveData = [filteredJiao[deriverName].tolist() for deriverName in cols]
  print("长度  ",len(deriveData), len(deriveData[0]))
  resData = {
    "data":                     data,
    "isf_name":                 "ISOF",
    "isf_resultLen":            len(isf_result),
